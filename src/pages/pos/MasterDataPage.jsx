...
import { Database, UtensilsCrossed, Package, Ruler } from 'lucide-react'
...
function countFoodCategory(name) {
  return MENU_ITEMS.filter(i => i.category === name).length
}

function countInventoryCategory(name) {
  return INVENTORY_ITEMS.filter(i => i.category === name).length
}

function countUnit(name) {
  return INVENTORY_ITEMS.filter(i => i.unit === name).length
}
  ...
  return (
    <div className="flex flex-col gap-5 max-w-3xl mx-auto">
      ...
      <div className="p-4 sm:p-5 rounded-2xl border
        ...
        {tab === 'food' && (
          <MasterDataListPanel
            ...
            getUsageCount={countFoodCategory}
        ...
        {tab === 'inventory' && (
          <MasterDataListPanel
            ...
            getUsageCount={countInventoryCategory}
      ...
      <p className="text-xs text-gray-400 dark:text-gray-600 text-center px-2">
        Changes apply immediately across Foods and Inventory forms. Delete is blocked while seed data still references an entry.
      </p>
    </div>
  )
}
