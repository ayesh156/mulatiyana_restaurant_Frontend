...
import {
  Plus, Pencil, Trash2, Check, X, AlertTriangle, Layers,
} from 'lucide-react'

// ─────────────────────────────────────────────────────────────────────────────
function DeleteConfirmModal({ name, itemCount, entityLabel, onConfirm, onCancel }) {
        ...
        <div className="p-6">
          {hasItems ? (
            <p className="text-gray-600 dark:text-gray-300 text-sm leading-relaxed">
              ...
              {' '}is referenced by {itemCount} record{itemCount !== 1 ? 's' : ''}.
              Reassign or remove those items before deleting.
            </p>
          ) : (
            <>
              <p className="text-gray-600 dark:text-gray-300 mb-4 text-sm">
                This {entityLabel.toLowerCase()} will be removed from lookup lists. This cannot be undone.
              </p>
...
// ─────────────────────────────────────────────────────────────────────────────
function ListRow({ name, itemCount, onRename, onDeleteRequest }) {
  ...
  function saveEdit() {
    ...
    const ok = onRename(name, trimmed)
        ...
        {editing ? (
          <>
            ...
            <button type="button" onClick={saveEdit}
              className="p-2 rounded-xl shrink-0 text-green-600 hover:bg-green-50 dark:hover:bg-green-500/10">
              <Check size={16} />
            </button>
  ...
  return (
    <>
        ...
        <div className="flex flex-col sm:flex-row gap-2">
          ...
          <button type="submit"
            className="flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl
                       font-semibold text-sm text-white shrink-0
      ...
      {pendingDelete && (
        <DeleteConfirmModal
          name={pendingDelete}
          itemCount={getUsageCount(pendingDelete)}
          entityLabel={entityLabel}
          onConfirm={() => {
            if (getUsageCount(pendingDelete) === 0) onDelete(pendingDelete)
